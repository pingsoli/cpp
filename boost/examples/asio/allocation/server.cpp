/*
 * We don't have a client testing program, but it's okay, telnet is our best
 * friend when using tcp protocol.
 *
 * the program is a echo server, sends back all received data to client.
 * <server> <port>
 *
 * the key point is memory allocation.
 */
#include <array>
#include <cstdlib>
#include <iostream>
 ...
 * requests. If the allocation is in use when an allocation request is made,
 ...
 * needs to statisfy the C++11 minimal allocator requirements.
 */
template <typename T>
...
private:
  ...
  // the underlying memory, NOTE: is reference, so it's not owned by
  // handler_allocator, in UML is called aggregation.
  handler_memory& memory_;
};

/*
 * Wraper class template for handler objects to allow handler memory
 * allocation to be customised. The allocator_type type ang get_allocator()
 * member function are used by asynchrounous operations to obtain the
